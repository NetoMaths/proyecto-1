...
st.title("Evaluación del Riesgo Financiero de Liverpool")
...
###################################################### Eficinecia  ###############################################################
   ...
   # Crear un DataFrame con los resultados sin los índices
    ...
    q_95 = norm.ppf(0.05)  #ESTE CODIGO CALCULA EL CUANTIL ALPHA=0.05 DE UNA NORMAL ESTANDAR 
    q_99 = norm.ppf(0.01)  #CALCULA LO MISMO PERO PAR AUN ALPHA DE 0.01
...
# Rolling VaR con volatilidad móvil
rolling_std = df_rendimientos[stock_seleccionado].rolling(window=252).std()
VaR_95_vol = q_95 * rolling_std
VaR_99_vol = q_99 * rolling_std

# Gráfico
plt.plot(VaR_95_vol, label='VaR 95% Vol Móvil')
plt.plot(VaR_99_vol, label='VaR 99% Vol Móvil')
